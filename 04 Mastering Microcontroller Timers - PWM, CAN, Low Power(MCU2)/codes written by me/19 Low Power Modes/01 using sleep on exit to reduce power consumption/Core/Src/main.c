...
int main(void)
{
	...
	//SCB->SCR |= (1 << 1);
	// before using next line , power consumption = 4.6 mA
	// after using next line , power consumption = 3.8 mA
	HAL_PWR_EnableSleepOnExit();

	/*let's start with fresh status register of timer to avoid any spurious interrupts */
	TIM6->SR = 0;

	// let's start timer in interrupt mode
